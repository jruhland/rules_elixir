load("@rules_elixir//impl:defs.bzl", "elixir_library", "elixir_script", "mix_project")

def autogenerated_targets():
  mix_project(
    name = "umbrella",
    mix_env = "dev",
    config_path = "config/config.exs",
    deps_path = "deps",
    deps_names = ["jason", "mimerl"],
    apps_names = ["app_one", "app_two", "common", "only_runtime"],
    lib_targets = ["//test/mix/umbrella/apps/app_one/lib:app_one",
                   "//test/mix/umbrella/apps/only_runtime/lib:only_runtime",
                   "//test/mix/umbrella/apps/app_two/lib:some_module",
                   "//test/mix/umbrella/apps/common/lib:common",
                   "//test/mix/umbrella/apps/common/lib:macros",
                   "//test/mix/umbrella/apps/common/lib:compiletime_util",
                   "//test/mix/umbrella/apps/app_two/lib:app_two",
                   "//test/mix/umbrella/apps/app_one/lib:macros"],
    apps_path = "apps",
    build_path = "_build/dev"
  )

